...
class CourseRemoteDataSourceImpl extends CourseRemoteDataSource {
  ...
  @override
  Future<BaseNetworkResponse<Video>> getVideoBySlug(String slug) async{
    var dio = BaseBrain.dio;
    var url = "${ApiEndpoints.videos}$slug";
    var result = await dio
        .get(
      url,
    )
        .then((value) {
      ResponseDtoUseCase response = ResponseDtoUseCase.fromJson(value.data);
      var course = Course.fromJson(response.data!);
      return BaseNetworkResponse<Video>(
          data: course.currentVideo,
          message: response.message);
    });
